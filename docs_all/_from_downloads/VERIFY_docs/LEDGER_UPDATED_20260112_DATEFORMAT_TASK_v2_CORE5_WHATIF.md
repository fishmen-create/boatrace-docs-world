...
### 運用ルール（このセクションの使い方）
- 当チャットで顕在化したタスクは **ここで状態管理**する（LEDGERが唯一のタスク正本）。
- status は次の4つに固定：`DONE / TODO / HOLD / DEFER`
- 更新は「行の status と updated を変える」だけ（削除しない）。
- priority は `P0 / P1 / P2 / P3` を使用（P0が最優先）。
- 命名規約タスク（NC-xx）は下段の「命名規約関連タスク台帳」を参照（状態はそちらが正）。

### 一覧（状態管理テーブル）

| id | priority | area | task | status | updated | trigger / note |
|---:|:---:|---|---|---|---|---|
| T-01 | P0 | OPS/AUTO | 安全設計付きワンボタン化（payout二重起動防止、b_only分岐、正本入力優先、期間寄せ、dedup+preflight+backtest統合） | TODO | 2026-01-13 | “準備→検証→出力”を一本道に。設計レビュー→段階導入が前提 |
| T-02 | P1 | UX/CLI | 日付引数の自動正規化（YYYYMMDD → YYYY-MM-DD をスクリプト側で吸収） | TODO | 2026-01-13 | 手順入力ミス（世界線ズレに見える）を根絶 |
| T-03 | P1 | VERIFY/GUARD | 期間カバレッジチェック（payouts × features の日付・場の欠損検知） | TODO | 2026-01-13 | preflightの前段ガード。欠損時はFAILで止める |
| T-04 | P1 | CONFIG | 除外場の設定ファイル化（引継ぎ必須のsource_of_truth整備） | TODO | 2026-01-13 | exclusions系の世界線ズレ防止。どのjsonが正か固定 |
| T-05 | P1 | GOVERN | 重要要素（未採用でも引継ぎ必須）の正本化・自動吸い上げ | TODO | 2026-01-13 | “未採用＝消える”事故防止（台帳→引継ぎ生成へ） |
| T-06 | P2 | PERF | payout収集の時間短縮（差分DLでも数時間かかる問題の改善） | TODO | 2026-01-13 | 差分・並列・キャッシュ等。時間コストがボトルネック |
| T-07 | P2 | STABILITY | backtest時の RuntimeWarning（Mean of empty slice）の是正 | TODO | 2026-01-13 | 欠損や空集合が原因。原因特定→防御実装 |

### 更新手順（最小）
1. 上表の該当行の `status` を更新（DONE/TODO/HOLD/DEFER）
2. 同じ行の `updated` を当日に更新（YYYY-MM-DD）
3. 追加タスクが出たら `T-08…` を追加（削除しない）
...
- [P1] core5（15場除外）の算出条件を明文化（=再現可能に固定）
  - 背景（簡潔）：除外場の“正”が曖昧だと、検証結果・運用判断が世界線ズレしやすい
  - 修正内容（具体）：①入力に使うファイル（例：BL_*_summary_bytrack.csv 等）②指標（ROI/Hit率/投資額など）③ソート順④除外上限N（=15）⑤同率/欠損の扱い⑥出力JSON名 をドキュメントに固定
- [P1] _whatif/ 配下のファイル命名規約を固定（比較軸が即判別できる形）
  - 背景（簡潔）：what-if出力が増えると「どの条件で作ったファイルか」が迷子ポイントになる
  - 修正内容（具体）：`WHATIF_{window}_{mode}_{policy}_{cap}_{excl}_{tag}_YYYYMMDD_HHMMSS.*` など、最低限 (window/mode/policy/cap/excl) を必須にする

---

## 命名規約（Naming Convention）関連タスク台帳（検証チャット7）

### 運用ルール（このセクションの使い方）
- 命名規約関連のタスクは **このセクションだけ**で状態管理する（RUNBOOK/HANDOVERには書かない）。
- 更新は「行の status を変える」だけで済む形にする（消し込み運用はしない）。
- status は次の4つに固定：`DONE / TODO / HOLD / DEFER`
  - DONE：完了（運用・比較に使える状態）
  - TODO：実施予定（着手前/実装中）
  - HOLD：条件待ち（トリガー待ち、保留）
  - DEFER：優先度を下げた（当面やらないが、消さない）
- 更新時は `updated`（YYYY-MM-DD）だけ必ず書き換える。

### 一覧（状態管理テーブル）

| id | area | task | status | updated | trigger / note |
|---:|---|---|---|---|---|
| NC-01 | CORE | 骨格定義：WORLD必須、WINDOW固定、VARIANT必須、OPT末尾追加のみ | DONE | 2026-01-13 | 正本：20_VERIFY/NAMING_CONVENTION_CORE.md |
| NC-02 | 90D | _whatif フォルダ命名規約（BR__VERIFY__WHATIF__...） | DONE | 2026-01-13 | 正本：20_VERIFY/NAMING_CONVENTION_90D_WHATIF.md |
| NC-03 | 90D | OPT語彙固定（CAP/EXCL/THEME）＋推奨並び | DONE | 2026-01-13 | 90D正本に反映済み |
| NC-04 | 90D | フォルダ内ファイル名固定（MANIFEST/roi_overall/roi_bytrack/command、BT=tickets） | DONE | 2026-01-13 | 90D正本に反映済み（FILELOCK） |
| NC-05 | 90D | Preflight：チェックリスト明文化＋簡易自動化（PASS/FAIL CSV） | DONE | 2026-01-13 | tools/preflight_check_whatif.py|ps1 + Runbook追記 |
| NC-06 | C | Cフェーズ命名規約（骨格のみ） | DONE | 2026-01-13 | 20_VERIFY/NAMING_CONVENTION_C_PHASE.md（骨格固定） |
| NC-07 | C | Cフェーズ suffix語彙（TRACK/INPUTSET/…）の確定 | HOLD | 2026-01-13 | トリガー：直近10運用日で出力が安定 or 体系確定 |
| NC-08 | C | C成果物ファイル名固定（C_OUT/diagnostic/tickets等） | HOLD | 2026-01-13 | トリガー：C_OUT構造が一本道で安定 |
| NC-09 | ALL | 命名規約の適用範囲拡張（90D限定 → 運用全体） | HOLD | 2026-01-13 | トリガー：運用Runbookが一本道で完走可能＋正本確定 |

### 更新手順（最小）
1. 上表の該当行の `status` を更新（DONE/TODO/HOLD/DEFER）
2. 同じ行の `updated` を当日に更新（YYYY-MM-DD）
3. 追加タスクが出たら、新しい id（NC-10…）を追加して行を1つ増やす（削除しない）
