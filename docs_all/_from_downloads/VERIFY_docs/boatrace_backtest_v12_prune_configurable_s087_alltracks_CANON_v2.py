...
def main() -> None:
    ...
    if args.mode == "analysis":
        ...
        # NOTE:
        # argparse の default が入っているだけ（daily_cap=0 / exclude_jcd=""）ではエラーにしない。
        # analysisで禁止したいのは「運用制約を有効化する指定」だけ。
        if args.daily_cap and int(args.daily_cap) > 0:
            raise SystemExit("[ERR] --mode analysis では --daily-cap を指定できません（運用制約はoperation側）")
        if isinstance(args.exclude_jcd, str) and args.exclude_jcd.strip() != "":
